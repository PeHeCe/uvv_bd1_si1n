...
CREATE USER pedrohc WITH CREATEDB 
                         CREATEROLE 
                         ENCRYPTED PASSWORD '17pehece';
SET ROLE pedrohc;
...
ALTER TABLE pedidos
ADD CONSTRAINT cc_pedidos_status_check   CHECK (status IN (
                                                    'CANCELADO',
                                                    'COMPLETO', 
                                                    'ABERTO', 
                                                    'PAGO',
...
ALTER TABLE envios
ADD CONSTRAINT cc_envios_status_check    
...
ALTER TABLE lojas
ADD CONSTRAINT cc_lojas_endereco_preenchido 
...
-- Verificação para garantir que o numero_da_linha seja maior ou igual a zero
...
-- Verificação para garantir que o telefone1 esteja dentro do padrão:  "(XX) XXXX-XXXX"
ALTER TABLE clientes
ADD CONSTRAINT cc_clientes_telefone1
CHECK (telefone1 ~ '^\([0-9]{2}\) [0-9]{4,5}-[0-9]{4}$');


-- Verificação para garantir que o telefone2 esteja dentro do padrão:  "(XX) XXXX-XXXX"
ALTER TABLE clientes
ADD CONSTRAINT cc_clientes_telefone2
CHECK (telefone2 ~ '^\([0-9]{2}\) [0-9]{4,5}-[0-9]{4}$');


-- Verificação para garantir que o telefone3 esteja dentro do padrão:  "(XX) XXXX-XXXX"
ALTER TABLE clientes
ADD CONSTRAINT cc_clientes_telefone3
CHECK (telefone3 ~ '^\([0-9]{2}\) [0-9]{4,5}-[0-9]{4}$');
